...
export function SettingsPage() {
  ...
  const storageMB = storageInfo
    ? (storageInfo.used / 1024 / 1024).toFixed(1)
    : "0";
  const quotaMB = storageInfo
    ? (storageInfo.quota / 1024 / 1024).toFixed(0)
    : "0";
  ...
  const handleExport = async () => {
    try {
      const [notes, routines, records, streak, quotes, labels] =
        await Promise.all([
          db.notes.toArray(),
          db.routines.toArray(),
          db.records.toArray(),
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      if (data.streak?.length)
        await db.streak.bulkAdd(
          data.streak.map((s: Record<string, unknown>) => {
            const { id: _, ...rest } = s;
            return rest;
          }),
        );
    ...
    <div className="p-4 max-w-lg mx-auto pb-8">
            ...
            {storagePercent > 80 && (
              <div className="flex items-center gap-2 mt-2 p-2 bg-destructive/10 rounded-lg">
                ...
                <p className="text-xs text-destructive">
                  Storage is nearly full. Consider exporting and clearing old
                  data.
                </p>
              </div>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl border border-border/50 p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-muted-foreground text-center mb-4">
              This will overwrite all existing data. This action cannot be
              undone.
            </p>
      ...
      <section className="bg-card rounded-xl border border-destructive/30 p-4">
        ...
        {clearStep === 2 && (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground">
              Type <strong>DELETE ALL</strong> to confirm permanent deletion.
            </p>
            <input
              type="text"
