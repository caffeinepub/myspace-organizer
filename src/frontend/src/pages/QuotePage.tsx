...
import { ArrowLeft, Calendar, Plus, Save, Shuffle, Trash2 } from "lucide-react";
...
export function QuotePage({ onBack }: QuotePageProps) {
  ...
  const [rotateMode, setRotateMode] = useState<"none" | "shuffle" | "daily">(
    "none",
  );
  ...
  const addToList = () => {
    ...
    setQuoteList((prev) => [
      ...prev,
      { text: newQuoteText.trim(), author: newQuoteAuthor.trim() || undefined },
    ]);
    setNewQuoteText("");
    setNewQuoteAuthor("");
  };
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      >
        ...
        {author && (
          <p
            className="text-sm text-muted-foreground mt-2 not-italic"
      ...
      <div className="bg-card rounded-xl border border-border/50 p-4 space-y-4 mb-4">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <span
              className={`block w-4 h-4 rounded-full bg-white shadow transition-transform mx-0.5 ${backgroundBlur ? "translate-x-5" : "translate-x-0"}`}
            />
          </button>
        </div>
      ...
      <div className="bg-card rounded-xl border border-border/50 p-4 mb-4">
        <h3 className="text-sm font-semibold mb-3">
          Quote Library ({quoteList.length})
        </h3>
        <div className="space-y-2 mb-3 max-h-48 overflow-y-auto">
          {quoteList.map((q, idx) => (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: quote list has no stable id; position is the only identity
              key={idx}
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              size="sm"
              className="h-8 gap-1"
              aria-label="Add quote to library"
            >
      ...
      <Button
        ...
        aria-label="Save quote settings"
      >
