...
export function useRoutines() {
  ...
  const saveProfile = useCallback(async (profile: RoutineProfile) => {
    try {
      if (profile.id) {
        ...
        // biome-ignore lint/style/noParameterAssign: need to update profile with generated id before using below
        profile = { ...profile, id };
      }
  ...
  const addItem = useCallback(
    async (profileType: ProfileType, item: RoutineItem) => {
  ...
  const updateItem = useCallback(
    async (profileType: ProfileType, item: RoutineItem) => {
      ...
      const updated = {
        ...profile,
        items: profile.items.map((i) => (i.id === item.id ? item : i)),
      };
      await saveProfile(updated);
    },
    [weekday, weekend, saveProfile],
  ...
  const deleteItem = useCallback(
    async (profileType: ProfileType, itemId: string) => {
      const profile = profileType === "weekday" ? weekday : weekend;
      if (!profile) return;
      try {
        const updated = {
  ...
  const reorderItems = useCallback(
    async (profileType: ProfileType, items: RoutineItem[]) => {
      const profile = profileType === "weekday" ? weekday : weekend;
      if (!profile) return;
      try {
