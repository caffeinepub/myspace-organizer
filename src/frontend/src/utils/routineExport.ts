...
function formatProfileForTxt(profile: RoutineProfile): string {
  ...
  const items = (profile.items || [])
    .map((item) => formatItemForTxt(item, profile.profileType))
    .join("\n");
  return [
    sep,
    `Profile: ${pLabel}`,
...
function formatItemForDoc(item: RoutineItem, pType: string): string {
  const tag = item.tag
    ? `<p><strong>Tag:</strong> ${escapeHtml(item.tag)}</p>`
    : "";
  const icon = item.icon
    ? `<p><strong>Icon:</strong> ${escapeHtml(item.icon)}</p>`
    : "";
  const duration =
...
function formatProfileForDoc(profile: RoutineProfile): string {
  const pLabel = profileLabel(profile.profileType);
  const items = (profile.items || [])
    .map((item) => formatItemForDoc(item, profile.profileType))
    .join("");
  return `
